...
public class Auth0HttpService implements AuthorizationHttpService {
    ...
    private String getAccessToken() {
        final HttpResponse<JsonNode> response = Unirest.post(auth0Data.tokenUrl())
    ...
    @Override
    public void createNewUser(final String username, final String password, final String inviterUsername) {
    ...
    private Map<String, Object> createNewUserRequestBody(final String username, final String password, final String inviterUsername) {
        final Map<String, Object> body = new HashMap<>();
    ...
    private void handleCreateNewUserResponse(final HttpResponse<JsonNode> response, final String username) {
        //TODO handle errors GCC-31
        switch (response.getStatus()) {
            case HttpStatus.CREATED -> log.info("User {} created", username);
            case HttpStatus.CONFLICT -> throw new RuntimeException("%s already exists".formatted(username));
            default -> throw new RuntimeException("Failed to create user: %s; Error: %s".formatted(username, response.getBody()));
        }
    }
