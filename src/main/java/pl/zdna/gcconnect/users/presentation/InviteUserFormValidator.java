...
public class InviteUserFormValidator implements Validator {
    ...
    public void validate(final @NonNull Object target, final @NonNull Errors errors) {
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, InviteUserForm.Fields.phoneNumber, "phoneNumber.invalid", "phone number is required");
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, InviteUserForm.Fields.username, "username.invalid", "username is required");
        ...
        if(vgnFactory.isNotValid(VGNType.PHONE_NUMBER, inviteUserForm.getPhoneNumber())){
            errors.rejectValue(InviteUserForm.Fields.phoneNumber, "phoneNumber.invalid","phone number is not a valid number");
        }

        if(vgnFactory.isNotValid(VGNType.USERNAME, inviteUserForm.getUsername())){
            errors.rejectValue(InviteUserForm.Fields.username, "username.invalid", "this username is not valid");
        }
    }
}
